...
$cor_recuperarUsuario = $_POST['gmail'];
$token_recuperar = $_POST['token'];
$codigo = $_POST['codigo'];

$sql = "CALL BuscarRecuperarContrasena(:correo_recuperar, :token_recuperar, :codigo_recuperar)";
$stmt = $pdo->prepare($sql);
$stmt->bindParam(':correo_recuperar', $cor_recuperarUsuario);
$stmt->bindParam(':token_recuperar', $token_recuperar);
$stmt->bindParam(':codigo_recuperar', $codigo);
$stmt->execute();
$correcto = false;
...
if ($stmt->rowCount() > 0) {
    ...
    $fecha = $fila['fecha_recuperar']; 

    $fecha_actual = time();

    $fecha_almacenada = strtotime($fecha);

    $segundos = $fecha_actual - $fecha_almacenada;
    ...
    // Establecer $correcto como true
    $correcto = true;
} else {
    // Si no hay resultados, $correcto se mantiene como false
    $correcto = false;
...
<head>
    ...
    <link rel="stylesheet" href="../css/contraseña_rec.css">
...
<header>
    ...
    <div class="wrapper">
        ...
        <div class="form-box login">
            ...
            <!-- Comprobar si $correcto es verdadero antes de mostrar el formulario -->
            <?php if ($correcto) { ?>
                <form action="../../Controlador/ctrlcambioConUsuario.php" method="POST">
                     <div class="input-box">
                     <span></span>
                        <label class="stilo_l">Contraseña</label>
                        <p class="stilo_p">Ingresa tu contraseña</p>
                        <input class="inp" type="password" id="password" name="pass1"
                            placeholder="Ingresa tu contraseña">
                    </div>
                    <div class="input-box">
                        <label class="stilo_l">Confirmar contraseña</label>
                        <p class="stilo_p">Confirma la contraseña ingresada</p>
                        <input class="inp" type="password" id="confirmPassword" name="pass2" placeholder="Confirma la contraseña">
                        <input type="hidden" name="gmail" value="<?php echo $cor_recuperarUsuario;?>">
                    </div>
                    <p id="message" style="color: red;"></p> 
                     <button type="submit" class="btn">Restablecer</button>   
                </form>
            <?php } else { ?>
                <!-- Mostrar un mensaje de error si $correcto es falso -->
   ...
   <script src="../js/restriccion_recuperarCon.js"></script>
</body>

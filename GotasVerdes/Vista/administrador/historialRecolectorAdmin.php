<?php
  ...
  if(isset($_GET['nombre'])) {
    ...
    include '../../controlador/ctrlIdRecolector.php'; // asegúrate de proporcionar la ruta correcta al archivo
    $idRecolector; // Asegúrate de asignar un valor a esta variable o eliminar la declaración si no es necesaria
    $stmt = $pdo->prepare("CALL recolectas_completas(?)");
    $stmt->bindParam(1, $idRecolector, PDO::PARAM_INT);
    $stmt->execute();
...
<body>
  <div class="container mt-3">
    <table class="table">
      ...
      <tbody>
        <?php 
          if(isset($recolectas) && is_array($recolectas)) {
